  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(NameDetailsPage).map { name =>
      ...
      SummaryListRowViewModel(
        key = "addYourName.checkYourAnswersLabel",
        value = ValueViewModel(HtmlContent(value)),
        actions = Seq(
          ActionItemViewModel("site.change", routes.AddYourNameController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("addYourName.change.hidden"))
        )
      )
    }
}
