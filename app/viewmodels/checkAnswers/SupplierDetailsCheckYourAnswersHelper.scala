...
object SupplierDetailsCheckYourAnswersHelper {
  ...
  private def buildRows(answers: UserAnswers, supplierNumber: SupplierNumber)(implicit messages: Messages) = {
    ...
    if (isUsingPersonalDetails(answers, supplierNumber)) {
      ...
      Seq(
        SupplierNameSummary.rowFromPurchaserDetails(
          answers,
          supplierNumber
        ), // TODO: Can pass in the page / name details. Will also need to determine the change redirect page
      ...
      Seq(
        ...
        SupplierNameSummary.rowFromSupplierDetails(
          answers,
          supplierNumber
        ), // TODO: Update Unit tests. Already existed but will need to change with new row methods
        SupplierAddressSummary.rowFromSupplierDetails(
          answers,
          supplierNumber
        ), // TODO: Unit tests, see other Summary examples. Also do all 3 row types
