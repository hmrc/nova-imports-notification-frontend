...
object PurchaserBusinessOrIndividualSummary {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PurchaserBusinessOrIndividualPage).map { answer =>
      ...
      SummaryListRowViewModel(
        key = "purchaserBusinessOrIndividual.checkYourAnswersLabel",
        value = ValueViewModel(value),
        actions = Seq(
          ActionItemViewModel("site.change", routes.PurchaserBusinessOrIndividualController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("purchaserBusinessOrIndividual.change.hidden"))
        )
      )
    }
}
