...
import pages.SpreadsheetValidationErrorsPage
...
class UploadSpreadsheetErrorDataController @Inject() (
                                                       val controllerComponents: MessagesControllerComponents,
                                                       actions: Actions,
                                                       view: UploadSpreadsheetErrorDataView
                                                     ) extends BaseController {

  import UploadSpreadsheetErrorDataController.*

  def onPageLoad(): Action[AnyContent] = actions.authAndGetDataWithUserTypeGuard(guardPredicate) { implicit request =>
    val validationErrors = request.userAnswers.get(SpreadsheetValidationErrorsPage).getOrElse(Seq.empty)
    Ok(view(validationErrors))
  }
}

object UploadSpreadsheetErrorDataController {
