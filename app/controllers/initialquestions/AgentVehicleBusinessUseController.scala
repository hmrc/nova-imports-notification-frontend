...
import pages.sections.initialquestions.{AgentClientVehicleBusinessUsePage, VehicleFromEuPage}
...
class AgentVehicleBusinessUseController @Inject() (
  val controllerComponents: MessagesControllerComponents,
  sessionRepository: SessionRepository,
  navigator: Navigator,
  actions: Actions,
  formProvider: AgentVehicleBusinessUseFormProvider,
  view: AgentVehicleBusinessUseView
  ...
  def onSubmit(mode: Mode): Action[AnyContent] =
    actions.novaAgentAuthAndGetDataRequiringClientWithGuard(guardPredicate).async { implicit request =>
      form
        .bindFromRequest()
        .fold(
