...
class PurchaserAddressSummarySpec extends SpecBase {
  ...
  "PurchaserAddressSummary" - {
    ...
    "must end a UK address with the postcode and never show the country name" in {
      ...
      val result = PurchaserAddressSummary.row(userAnswers).value
      val value  = result.value.content.asHtml.toString

      result.key.content.asHtml.toString must include(msgs("purchaserAddressCheckYourAnswers.checkYourAnswersLabel"))
      value mustBe "12 High Street<br>Reading<br>RE12 9GC"
      result.actions.value.items.head.href mustBe routes.PurchaserAddressCheckYourAnswersController.onChangeAddress().url
    }
