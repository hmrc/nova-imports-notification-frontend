  ...
  "Actions" - {
    ...
    "vatAgentAuthAndGetData" - {
      ...
      "must execute the block when user data exists" in {
        ...
        running(application) {
          val actions: Actions = application.injector.instanceOf[Actions]
          val action           = actions.vatAgentAuthAndGetData()((_: DataRequest[AnyContent]) => Results.Ok("success"))
          val result           = action.apply(FakeRequest())
    ...
    "novaAgentAuthAndGetData" - {
      ...
      "must execute the block when user data exists" in {
        ...
        running(application) {
          val actions: Actions = application.injector.instanceOf[Actions]
          val action           = actions.novaAgentAuthAndGetData()((_: DataRequest[AnyContent]) => Results.Ok("success"))
          val result           = action.apply(FakeRequest())
    ...
    "novaAgentAuthAndGetDataRequiringClient" - {

      "must execute the block when user is not an agent" in {
        given application: Application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()

        running(application) {
          val actions: Actions = application.injector.instanceOf[Actions]
          val action           = actions.novaAgentAuthAndGetDataRequiringClient()((_: DataRequest[AnyContent]) => Results.Ok("success"))
          val result           = action.apply(FakeRequest())
      ...
      "must execute the block when agent has a selected client" in {
        val answersWithClient          = emptyUserAnswers.set(AgentSelectedClientPage, AgentSelectedClient("123456789")).success.value
        given application: Application = applicationBuilderWithAgentAsNovaAgent(userAnswers = Some(answersWithClient)).build()
      ...
      "must redirect to Journey Recovery when agent has no selected client" in {
        given application: Application = applicationBuilderWithAgentAsNovaAgent(userAnswers = Some(emptyUserAnswers)).build()
