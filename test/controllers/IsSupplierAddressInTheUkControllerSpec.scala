...
class IsSupplierAddressInTheUkControllerSpec extends SpecBase with MockitoSugar {
  ...
  private lazy val isSupplierAddressInTheUkRoute = routes.IsSupplierAddressInTheUKController.onPageLoad(SupplierNumber(1), NormalMode).url
  private def isSupplierAddressInTheUkSubmitRoute(supplierNumber: SupplierNumber, mode: Mode = NormalMode) =
    routes.IsSupplierAddressInTheUKController.onSubmit(supplierNumber, mode).url
