...
import models.{NormalMode, PurchaserBusinessOrIndividual, PurchaserOrOnBehalf, UserAnswers}
...
class PurchaserBusinessOrIndividualControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val purchaserBusinessOrIndividualRoute = initialquestions.routes.PurchaserBusinessOrIndividualController.onPageLoad(NormalMode).url
  ...
  "PurchaserBusinessOrIndividualController" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, purchaserBusinessOrIndividualRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[PurchaserBusinessOrIndividualView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(PurchaserBusinessOrIndividual.NonVatRegisteredBusiness), NormalMode)(
          request,
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, purchaserBusinessOrIndividualRoute)
            .withFormUrlEncodedBody(("value", PurchaserBusinessOrIndividual.NonVatRegisteredBusiness.toString))
    ...
    "must redirect to Journey Recovery for a GET if guard data is not met" in {
