...
class SupplierVatRegistrationDetailsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "SupplierVatRegistrationDetailsController" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, supplierVatRegistrationDetailsSubmitRoute(SupplierNumber(1)))
    ...
    "must save the answer matching that of the supplier number in the URL" in {
      ...
      val answersForSupplierThree = emptyUserAnswers
        ...
        .unsafeSet(AllSuppliersQuery, Map("3" -> Json.obj("usePersonalDetailsAsSupplier" -> false)))
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, supplierVatRegistrationDetailsRoute)
    ...
    "must redirect to Unauthorised for a POST if no existing data is found" in {
      ...
      running(application) {
        val request = FakeRequest(POST, supplierVatRegistrationDetailsRoute)
          .withFormUrlEncodedBody("value" -> validVatNumberDetails.toString)
        val result = route(application, request).value
    ...
    "must redirect to Unauthorised for a GET if AVD-S8.0 was answered No for data guard" in {
    ...
    "must redirect to Unauthorised for a GET if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request = FakeRequest(GET, supplierVatRegistrationDetailsSubmitRoute(SupplierNumber(2), NormalMode))
        val result  = route(application, request).value
    ...
    "must redirect to Unauthorised for a POST if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request = FakeRequest(POST, supplierVatRegistrationDetailsSubmitRoute(SupplierNumber(2), NormalMode))
          .withFormUrlEncodedBody("value" -> "true")
        val result = route(application, request).value
