...
class UploadSpreadsheetErrorUnknownControllerSpec extends SpecBase {

  private lazy val onPageLoadRoute = vehicledetails.routes.UploadSpreadsheetErrorUnknownController.onPageLoad().url
  ...
  "UploadSpreadsheetErrorUnknownController" - {

    "must return OK and render UVS3.1 for a VAT-registered organisation" in {
      val application = applicationFor(classOf[FakeVatTraderIdentifierAction], Some(answersWithDraftIdAndEuAnswer))

      running(application) {
        val request = FakeRequest(GET, onPageLoadRoute)
        val result  = route(application, request).value
        val view    = application.injector.instanceOf[UploadSpreadsheetErrorUnknownView]
