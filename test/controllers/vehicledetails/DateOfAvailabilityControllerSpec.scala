...
import pages.sections.vehicledetails.{DateOfAvailabilityPage, VehicleDatesPage}
...
class DateOfAvailabilityControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val dateOfAvailabilityRoute =
    vehicledetails.routes.DateOfAvailabilityController.onPageLoad(supplierNumber, vehicleNumber, NormalMode).url
  ...
  "DateOfAvailabilityController" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view      = application.injector.instanceOf[DateOfAvailabilityView]
        val appConfig = application.injector.instanceOf[FrontendAppConfig]
        val form      = new DateOfAvailabilityFormProvider()()(messages(application))

        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form, supplierNumber, vehicleNumber, NormalMode, appConfig.importingVehiclesIntoTheUKUrl)(
          request,
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = userAnswersWithGuardData
        .set(DateOfAvailabilityPage(supplierNumber, vehicleNumber), answer)
        .success
        .value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(answer), supplierNumber, vehicleNumber, NormalMode, appConfig.importingVehiclesIntoTheUKUrl)(
          request,
          messages(application)
        ).toString
      }
    ...
    "must redirect to the next page and save the date when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
        ...
        savedAnswers(mockSessionRepository).get(DateOfAvailabilityPage(supplierNumber, vehicleNumber)) mustEqual Some(answer)
      }
    }
    ...
    "must return a Bad Request and the empty date error when no date is entered" in {
      ...
      running(application) {
        ...
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.required.all"))
      }
    }
    ...
    "must return a Bad Request and the missing day error when only the day is missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
            .withFormUrlEncodedBody(dateFields("", "03", "2026")*)

        val result = route(application, request).value

        status(result) mustEqual BAD_REQUEST
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.required", messages(application)("date.error.day")))
      }
    }
    ...
    "must return a Bad Request and the missing month error when only the month is missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
            .withFormUrlEncodedBody(dateFields("27", "", "2026")*)

        val result = route(application, request).value

        status(result) mustEqual BAD_REQUEST
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.required", messages(application)("date.error.month")))
      }
    }
    ...
    "must return a Bad Request and the missing year error when only the year is missing" in {
      ...
      running(application) {
        ...
        status(result) mustEqual BAD_REQUEST
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.required", messages(application)("date.error.year")))
      }
    }
    ...
    "must return a Bad Request and the two-part error when the day and month are missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
            .withFormUrlEncodedBody(dateFields("", "", "2026")*)
        ...
        contentAsString(result) must include(
          messages(application)(
            "dateOfAvailability.error.required.two",
    ...
    "must return a Bad Request and the two-part error when the day and year are missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
            .withFormUrlEncodedBody(dateFields("", "03", "")*)
        ...
        contentAsString(result) must include(
          messages(application)(
            "dateOfAvailability.error.required.two",
            messages(application)("date.error.day"),
            messages(application)("date.error.year")
          )
        )
      }
    }
    ...
    "must return a Bad Request and the two-part error when the month and year are missing" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, dateOfAvailabilityRoute)
            .withFormUrlEncodedBody(dateFields("27", "", "")*)
    ...
    "must return a Bad Request and the format error when the date is not made up of numbers" in {
      ...
      running(application) {
        ...
        status(result) mustEqual BAD_REQUEST
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.invalid"))
      }
    }
    ...
    "must return a Bad Request and the real date error when the date does not exist" in {
      ...
      running(application) {
        ...
        status(result) mustEqual BAD_REQUEST
        contentAsString(result) must include(messages(application)("dateOfAvailability.error.notARealDate"))
      }
    }
    ...
    "must return OK for a GET when the date of availability was not one of the dates selected on AVD3.0" in {

      val answersWithoutDateOfAvailability = userAnswersWithGuardData
        .set(VehicleDatesPage(supplierNumber, vehicleNumber), Set(VehicleDates.PurchaseInvoiceDate))
        .success
        .value

      val application = applicationBuilder(userAnswers = Some(answersWithoutDateOfAvailability)).build()
    ...
    "must redirect to Unauthorised for a GET if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, vehicledetails.routes.DateOfAvailabilityController.onPageLoad(SupplierNumber(2), vehicleNumber, NormalMode).url)
    ...
    "must redirect to Unauthorised for a GET if the vehicle number in the URL is not one of the user's vehicles" in {
      ...
      running(application) {
        val request =
          FakeRequest(GET, vehicledetails.routes.DateOfAvailabilityController.onPageLoad(supplierNumber, VehicleNumber(999), NormalMode).url)
