...
class UpdateVehicleSpreadsheetControllerSpec extends SpecBase {

  private lazy val pageRoute = vehicledetails.routes.UpdateVehicleSpreadsheetController.onPageLoad().url

  private val form = new UpdateVehicleSpreadsheetFormProvider()()
  ...
  private val agentAnswersWithClient: UserAnswers =
    answersWithDraftIdAndEuAnswer.unsafeSet(AgentSelectedClientPage, AgentSelectedClient("700011916", Some("Client Co")))
  ...
  "UpdateVehicleSpreadsheetController" - {

    "onPageLoad" - {

      "must return OK and the correct view for a VAT-registered organisation" in {
        ...
        running(application) {
          ...
          val view    = application.injector.instanceOf[UpdateVehicleSpreadsheetView]
      ...
      "must return OK for an agent who has selected a client" in {
        val application = applicationFor(classOf[FakeAgentIdentifierAction], Some(agentAnswersWithClient))

        running(application) {
          val result = route(application, FakeRequest(GET, pageRoute)).value
      ...
      "must redirect to Unauthorised when the vehicle from EU question has not been answered" in {
        val application = applicationFor(classOf[FakeVatTraderIdentifierAction], Some(emptyUserAnswers.unsafeSet(DraftIdPage, DraftId("DRAFT-001"))))
      ...
      "must redirect to Unauthorised if draftId is missing" in {
        val application = applicationFor(classOf[FakeVatTraderIdentifierAction], Some(emptyUserAnswers.unsafeSet(VehicleFromEuPage, true)))
    ...
    "onSubmit" - {
      ...
      "must redirect to UVS1.0 Upload a vehicle spreadsheet when Yes is selected" in {
        val application = applicationFor(classOf[FakeVatTraderIdentifierAction], Some(answersWithDraftIdAndEuAnswer))

        running(application) {
          val request = FakeRequest(POST, pageRoute).withFormUrlEncodedBody("value" -> "true")
          val result  = route(application, request).value

          status(result) mustEqual SEE_OTHER
          redirectLocation(result).value mustEqual vehicledetails.routes.UploadVehicleSpreadsheetController.onPageLoad().url
        }
      }

      "must redirect to UVS5.0 Check the vehicle spreadsheet details when No is selected" in {
        val application = applicationFor(classOf[FakeAgentIdentifierAction], Some(agentAnswersWithClient))

        running(application) {
          val request = FakeRequest(POST, pageRoute).withFormUrlEncodedBody("value" -> "false")
          val result  = route(application, request).value

          status(result) mustEqual SEE_OTHER
          redirectLocation(result).value mustEqual vehicledetails.routes.CheckVehicleSpreadsheetDetailsController.onPageLoad().url
        }
      }
      ...
      "must return a Bad Request and errors when nothing is selected" in {
        ...
        running(application) {
          val request   = FakeRequest(POST, pageRoute).withFormUrlEncodedBody("value" -> "")
          ...
          contentAsString(result) must include(messages(application)("updateVehicleSpreadsheet.error.required"))
        }
      }
