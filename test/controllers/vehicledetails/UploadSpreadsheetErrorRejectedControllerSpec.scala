...
class UploadSpreadsheetErrorRejectedControllerSpec extends SpecBase {

  private lazy val onPageLoadRoute = vehicledetails.routes.UploadSpreadsheetErrorRejectedController.onPageLoad().url
  ...
  "UploadSpreadsheetErrorRejectedController" - {

    "must return OK and render UVS3.2 for a VAT-registered organisation" in {
      val application = applicationFor(classOf[FakeVatTraderIdentifierAction], Some(answersWithDraftIdAndEuAnswer))

      running(application) {
        val request = FakeRequest(GET, onPageLoadRoute)
        val result  = route(application, request).value
        val view    = application.injector.instanceOf[UploadSpreadsheetErrorRejectedView]
