...
import controllers.clientselection
import play.api.Application
import play.api.mvc.AnyContentAsEmpty
import play.api.test.FakeRequest
import play.api.test.Helpers.*

class LoadingClientListControllerSpec extends SpecBase {
    ...
    "onPageLoad" - {
      ...
      "must return OK and render the correct view" in {
        ...
        running(application) {
          given request: FakeRequest[AnyContentAsEmpty.type] =
            FakeRequest(GET, clientselection.routes.LoadingClientListController.onPageLoad().url)
      ...
      "must render the warning message" in {
        ...
        running(application) {
          ...
          status(result) mustEqual OK
          contentAsString(result) must include("Do not refresh this page or use the back button.")
        }
      }

      "must render the paragraph message" in {
        given application: Application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
