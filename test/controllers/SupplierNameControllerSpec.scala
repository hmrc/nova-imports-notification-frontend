...
class SupplierNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val supplierNameRoute = routes.SupplierNameController.onPageLoad(SupplierNumber(1), NormalMode).url
  ...
  "SupplierNameController" - {
    ...
    "must keep the same supplier number when the user returns to change their answer" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.SupplierNameController.onSubmit(SupplierNumber(3), CheckMode).url)
            .withFormUrlEncodedBody(("title", validTitle), ("firstName", validFirstName), ("lastName", validLastName))
    ...
    "must redirect to Unauthorised for a GET if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.SupplierNameController.onPageLoad(SupplierNumber(2), NormalMode).url)
    ...
    "must redirect to Unauthorised for a POST if the supplier number in the URL is not one of the user's suppliers" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.SupplierNameController.onSubmit(SupplierNumber(2), NormalMode).url)
            .withFormUrlEncodedBody(("title", validTitle), ("firstName", validFirstName), ("lastName", validLastName))
