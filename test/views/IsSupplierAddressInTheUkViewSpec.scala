...
class IsSupplierAddressInTheUkViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {

  val app: Application             = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
  implicit val request: Request[?] = FakeRequest()
  implicit val msgs: Messages      = messages(app)

  val view: IsSupplierAddressInTheUkView = app.injector.instanceOf[IsSupplierAddressInTheUkView]
  ...
  val formProvider = new IsSupplierAddressInTheUkFormProvider()
  val form         = formProvider()

  "IsSupplierAddressInTheUkView" - {

    "must render the correct heading" in {
      val html: String = view(form, SupplierNumber(1), NormalMode)(request, msgs).toString

      html must include(msgs("isSupplierAddressInUK.heading"))
    }

    "must render the correct page title" in {
      val html: String = view(form, SupplierNumber(1), NormalMode)(request, msgs).toString

      html must include(msgs("isSupplierAddressInUK.title"))
    }
    ...
    "must render the correct page caption" in {
      ...
      html must include(msgs("isSupplierAddressInUK.caption"))
    }
    ...
    "must render the error summary when the form has errors" in {
      ...
      html must include(msgs("isSupplierAddressInUK.error.required"))
    }
