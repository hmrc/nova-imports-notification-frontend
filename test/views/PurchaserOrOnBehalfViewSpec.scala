...
class PurchaserOrOnBehalfViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {
  ...
  "PurchaserOrOnBehalfView" - {
    ...
    "must render the purchaser radio option" in {
      val html: String = view(form, NormalMode)(request, msgs).toString

      html must include(msgs("purchaserOrOnBehalf.radio.purchaser"))
    }

    "must render the on behalf of purchaser radio option" in {
      val html: String = view(form, NormalMode)(request, msgs).toString
