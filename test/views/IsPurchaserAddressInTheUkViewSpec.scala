...
class IsPurchaserAddressInTheUkViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {

  val app: Application             = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
  implicit val request: Request[?] = FakeRequest()
  implicit val msgs: Messages      = messages(app)

  val view: IsPurchaserAddressInTheUkView = app.injector.instanceOf[IsPurchaserAddressInTheUkView]
  ...
  "IsPurchaserAddressInTheUkView" - {

    "must render the correct heading" in {
      val html: String = view(form, NormalMode)(request, msgs).toString

      html must include(msgs("isPurchaserAddressInTheUk.heading"))
    }

    "must render the correct page title" in {
      val html: String = view(form, NormalMode)(request, msgs).toString

      html must include(msgs("isPurchaserAddressInTheUk.title"))
    }
    ...
    "must render the 'Add purchaser address' caption" in {
      ...
      html must include(msgs("isPurchaserAddressInTheUk.caption"))
    }
    ...
    "must render the error summary when the form has errors" in {
      ...
      html must include(msgs("isPurchaserAddressInTheUk.error.required"))
    }
