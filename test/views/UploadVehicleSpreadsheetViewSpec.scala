...
class UploadVehicleSpreadsheetViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {
  ...
  private def render: String = view(uploadUrl, fields, spreadsheetUrl)(request, msgs).toString.replaceAll("\\s+", " ")
  ...
  "UploadVehicleSpreadsheetView" - {
    ...
    "must name the file input file and mark it required for users without javascript" in {
      val html = render
      html must include("""<input class="govuk-file-upload" id="file" name="file" type="file"""")
      html must include("""required="required"""")
    }
