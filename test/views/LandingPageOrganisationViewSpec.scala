...
class LandingPageOrganisationViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {
  ...
  "LandingPageOrganisationView" - {
    ...
    "must render the trader name caption" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
      html must include(traderName)
      html must include("govuk-caption-l")
    }

    "must render the VRN caption with GB prefix" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
    ...
    "must not render a trader name caption when traderName is None" in {
      val html: String = view(None, vrn, hasDraftNotifications = false).toString
      html must not include traderName
      html must include(msgs("landingPage.organisation.vrn.caption", vrn))
    }

    "must render the body intro" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
      html must include(msgs("landingPage.organisation.body"))
    }

    "must render the create-a-new-notification section linking to /start" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
    ...
    "must render the update-a-submitted-notification section linking to /start" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
      html must include(msgs("landingPage.organisation.update.link"))
      html must include(msgs("landingPage.organisation.update.body"))
    }

    "must render the manage-a-saved-notification heading" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
      html must include(msgs("landingPage.organisation.saved.heading"))
    }

    "must render the empty saved-notifications message when hasDraftNotifications is false" in {
      val html: String = view(Some(traderName), vrn, hasDraftNotifications = false).toString
    ...
    "must render the has-drafts saved-notifications message when hasDraftNotifications is true" in {
