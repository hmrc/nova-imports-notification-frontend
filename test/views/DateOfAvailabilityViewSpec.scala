...
class DateOfAvailabilityViewSpec extends SpecBase with Matchers with BeforeAndAfterAll {

  val app: Application             = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
  implicit val request: Request[?] = FakeRequest()
  implicit val msgs: Messages      = messages(app)

  val view: DateOfAvailabilityView = app.injector.instanceOf[DateOfAvailabilityView]
  ...
  val form: Form[LocalDate] = new DateOfAvailabilityFormProvider()()

  val importingVehiclesIntoTheUKUrl = "http://example.com/importing-vehicles-into-the-uk"

  private def render(form: Form[LocalDate] = form): String =
    view(form, SupplierNumber(1), VehicleNumber(1), NormalMode, importingVehiclesIntoTheUKUrl)(request, msgs).toString
  ...
  "DateOfAvailabilityView" - {

    "must render the correct H1" in {
      val document = Jsoup.parse(render())

      document.select("h1").text mustEqual msgs("dateOfAvailability.heading")
      msgs("dateOfAvailability.heading") mustEqual "Date of availability"
    }

    "must render the correct page title" in {
      Jsoup.parse(render()).title mustEqual msgs("dateOfAvailability.title") + " - " + msgs("service.name") + " - " + msgs("site.govuk")
      msgs("dateOfAvailability.title") mustEqual "Date of availability"
    }
    ...
    "must render the correct page caption" in {
      ...
      html must include(msgs("dateOfAvailability.caption"))
    }

    "must render the introductory paragraph" in {
      render() must include(msgs("dateOfAvailability.paragraph.1"))
    }

    "must render the inset text with a link to importing vehicles into the UK that opens in a new tab" in {
      val document = Jsoup.parse(render())

      val inset = document.select(".govuk-inset-text")
      inset.text must include(msgs("dateOfAvailability.inset.importingVehicles.linkText"))

      val link = inset.select("a")
      link.attr("href") mustEqual importingVehiclesIntoTheUKUrl
      link.attr("target") mustEqual "_blank"
      link.text must include(msgs("dateOfAvailability.inset.importingVehicles.linkText"))
    }

    "must render the question heading as an H2" in {
      val document = Jsoup.parse(render())

      document.select("h2.govuk-fieldset__heading").text mustEqual msgs("dateOfAvailability.question.heading")
    }
    ...
    "must show an error summary linking to the day field when the date is missing" in {
      ...
      document.select(".govuk-error-summary").text must include(msgs("dateOfAvailability.error.required.all"))
    ...
    "must highlight only the year field when only the year is missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required", msgs("date.error.year")))
    ...
    "must highlight only the month field when only the month is missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required", msgs("date.error.month")))
    ...
    "must highlight only the day field when only the day is missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required", msgs("date.error.day")))
    ...
    "must highlight the day and month fields when both are missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required.two", msgs("date.error.day"), msgs("date.error.month")))
      hasErrorClass(html, "day") mustEqual true
      hasErrorClass(html, "month") mustEqual true
      hasErrorClass(html, "year") mustEqual false
    }
    ...
    "must highlight the day and year fields when both are missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required.two", msgs("date.error.day"), msgs("date.error.year")))
      hasErrorClass(html, "day") mustEqual true
      hasErrorClass(html, "year") mustEqual true
      hasErrorClass(html, "month") mustEqual false
    }
    ...
    "must highlight the month and year fields when both are missing" in {
      ...
      html must include(msgs("dateOfAvailability.error.required.two", msgs("date.error.month"), msgs("date.error.year")))
      hasErrorClass(html, "month") mustEqual true
      hasErrorClass(html, "year") mustEqual true
      hasErrorClass(html, "day") mustEqual false
    }
    ...
    "must highlight only the day field when the day cannot be a day of any month" in {
      ...
      html must include(msgs("dateOfAvailability.error.notARealDate"))
      hasErrorClass(html, "day") mustEqual true
      hasErrorClass(html, "month") mustEqual false
      hasErrorClass(html, "year") mustEqual false
    }
    ...
    "must show the format error when the date is not made up of numbers" in {
      render(bind("aa", "03", "2026")) must include(msgs("dateOfAvailability.error.invalid"))
    }
