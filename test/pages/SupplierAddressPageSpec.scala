...
class SupplierAddressPageSpec extends SpecBase {
  ...
  "SupplierAddressPage" - {
    ...
    "must store the address at the supplier-details path" in {
      val answers = emptyUserAnswers.unsafeSet(SupplierAddressPage, address)

      (answers.data \ "supplier-details" \ "address").as[Address] mustBe address
    }
  }
}
